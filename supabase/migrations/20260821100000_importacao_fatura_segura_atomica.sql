-- ============================================================================
-- Migration: 20260821100000_importacao_fatura_segura_atomica.sql
-- Importação Atômica, Idempotente e Segura de Fatura de Cartão de Crédito
-- ============================================================================

-- 0. Garantir colunas de auditoria e rastreabilidade
DO $$ 
BEGIN
  -- fatura_cartao_importacoes
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'fatura_cartao_importacoes' AND column_name = 'total_lancamentos') THEN
    ALTER TABLE public.fatura_cartao_importacoes ADD COLUMN total_lancamentos NUMERIC(15,2);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'fatura_cartao_importacoes' AND column_name = 'total_fatura') THEN
    ALTER TABLE public.fatura_cartao_importacoes ADD COLUMN total_fatura NUMERIC(15,2);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'fatura_cartao_importacoes' AND column_name = 'ajustes_fatura') THEN
    ALTER TABLE public.fatura_cartao_importacoes ADD COLUMN ajustes_fatura NUMERIC(15,2);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'fatura_cartao_importacoes' AND column_name = 'hash_documento') THEN
    ALTER TABLE public.fatura_cartao_importacoes ADD COLUMN hash_documento TEXT;
  END IF;

  -- transacoes
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'transacoes' AND column_name = 'importacao_id') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'transacoes' AND column_name = 'hash_importacao') THEN
    ALTER TABLE public.transacoes ADD COLUMN hash_importacao TEXT;
  END IF;

  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'transacoes' AND column_name = 'numero_linha_importacao') THEN
    ALTER TABLE public.transacoes ADD COLUMN numero_linha_importacao INT;
  END IF;
END $$;

-- 1. Assegurar integridade e colunas de datas no cartão
DO $$ 
BEGIN
  IF NOT EXISTS (
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'contas_usuario' AND column_name = 'data_fechamento'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'contas_usuario' AND column_name = 'data_vencimento'
  ) THEN
    ALTER TABLE public.contas_usuario ADD COLUMN data_vencimento DATE;
  END IF;
END $$;

-- 2. Recriação dos Índices Únicos para Proteção Concorrente
...
-- 3. Drop de assinaturas anteriores da RPC
...
DROP FUNCTION IF EXISTS public.importar_fatura_atomica(UUID, UUID, TEXT, DATE, DATE, NUMERIC, NUMERIC, NUMERIC, TEXT, JSONB);

-- 4. Função RPC Endurecida com Atualização Atômica de Datas e Isolamento Rigoroso
CREATE OR REPLACE FUNCTION public.importar_fatura_atomica(
  ...
  p_transacoes JSONB,
  p_fechamento DATE DEFAULT NULL
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF p_workspace_id IS NOT NULL THEN
    ...
    -- Se o cartão for legado (workspace_id IS NULL), vincular ao workspace
    UPDATE public.contas_usuario
    SET workspace_id = p_workspace_id
    WHERE id = p_cartao_id AND workspace_id IS NULL;
  ELSE
  ...
  -- 3. Atualização Atômica das Datas no Cartão (quando informadas)
  IF p_vencimento IS NOT NULL OR p_fechamento IS NOT NULL THEN
    UPDATE public.contas_usuario
    SET 
      data_vencimento = COALESCE(p_vencimento, data_vencimento),
      data_fechamento = COALESCE(p_fechamento, data_fechamento)
    WHERE id = p_cartao_id;
  END IF;
...
REVOKE ALL ON FUNCTION public.importar_fatura_atomica(UUID, UUID, TEXT, DATE, NUMERIC, NUMERIC, NUMERIC, TEXT, JSONB, DATE) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.importar_fatura_atomica(UUID, UUID, TEXT, DATE, NUMERIC, NUMERIC, NUMERIC, TEXT, JSONB, DATE) TO authenticated;
