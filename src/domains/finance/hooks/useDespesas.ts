...
const DIVIPAY_SETTLED_STATUSES = [
  "FINISHED", "COMPLETED", "SETTLED", "PAID", "PAGO", "CONFIRMED",
];
...
/**
 * Classifica o meio de pagamento de saques/lançamentos da Divipay.
 *
 * Regras estritas de negócio:
 * 1. Não classificar pelo texto da descrição (w.description) — textos como "Pagamento de boleto"
 *    representam a finalidade declarada do saque na Divipay, não o meio de pagamento efetivo.
 * 2. Só usar 'pix' quando o tipo fornecido pela API for explicitamente 'DICT' ou 'PIX'.
 * 3. Só usar 'boleto' quando a API fornecer explicitamente o tipo/meio de pagamento boleto no contrato.
 * 4. Só usar 'cartao_credito' quando houver informação explícita da API de cartão de crédito.
 * 5. Quando a Divipay não informar o meio efetivo comprovado, classificar como 'outros'
 *    (rótulo "OUTROS" / "Saque Divipay Não Identificado").
 */
export function classificarMetodoPagamentoDivipay(saque?: {
  type?: string | null;
  ...
  // Se houver método explicitamente informado no contrato
  if (explicitMethod === "pix") return "pix";
  if (explicitMethod === "boleto") return "boleto";
  if (explicitMethod === "cartao_credito" || explicitMethod === "credit_card") return "cartao_credito";
  if (explicitMethod === "cartao_debito" || explicitMethod === "debit_card") return "cartao_debito";

  // Meios fornecidos explicitamente pelo type da API Divipay
  if (rawType === "DICT" || rawType === "PIX") {
    return "pix";
  }
  if (rawType === "CREDIT_CARD" || rawType === "CARTAO_CREDITO") {
    return "cartao_credito";
  }
  if (rawType === "DEBIT_CARD" || rawType === "CARTAO_DEBITO") {
    return "cartao_debito";
  }

  // Quando a Divipay não informar o meio efetivo (saques de liquidação, BILLET sem meio efetivo de cobrança, genéricos, etc.):
  return "outros";
}
...
async function fetchDivipayDespesas(startDate?: string | null, endDate?: string | null, workspaceId?: string | null): Promise<Despesa[]> {
  try {
    ...
    return allWithdraws
      .filter((w) => {
        const status = String(w.status || "").trim().toUpperCase();
        if (!status || !DIVIPAY_SETTLED_STATUSES.includes(status)) return false;
        if (DIVIPAY_NON_SETTLED_STATUSES.some((s) => status.includes(s))) return false;
      ...
      .map((w) => {
        const metodo = classificarMetodoPagamentoDivipay(w);
        const isPix = metodo === "pix";
        const resolved = resolveBeneficiary(Number(w.amount || 0), w.description || "", isPix ? "Pix" : null);
        const dateVal = formatarDataParaSaoPaulo(w.createdAt) || getHojeSaoPaulo();
        const desc = (w.description || "").trim() || (isPix ? "Saque Pix" : "Saque Divipay");
        const fav = w.name || resolved.name || "Divipay";
        const descricaoFinal = desc.includes(fav) ? desc : `${desc} - ${fav}`;
        const rotuloObservacao = isPix
          ? "Pix"
          : metodo === "cartao_credito"
            ? "Cartão Crédito"
            : metodo === "boleto"
              ? "Boleto"
              : "Saque Divipay Não Identificado";
        ...
        return {
          ...
          observacoes: `Pago via Divipay (${rotuloObservacao}) - ${w.id}`,
          status: "pago",
          categorias: {
