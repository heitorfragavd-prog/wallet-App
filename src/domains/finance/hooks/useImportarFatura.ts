...
function sugerirCategoria(descricao: string): string | undefined {
  ...
  for (const [categoria, keywords] of Object.entries(KEYWORD_CATEGORIES)) {
    for (const keyword of keywords) {
      if (descUpper.includes(keyword.toUpperCase())) return categoria;
    }
  }
  return undefined;
}
...
function hashTransacoes(transacoes: TransacaoParseada[]): string {
  const str = transacoes
    .map(t => `${t.data}|${t.descricao}|${t.valor}|${t.parcela_atual || 0}|${t.total_parcelas || 0}`)
    .sort().join(";;");
...
export function detectarBanco(texto: string): BancoDetectado {
  ...
  if (upper.includes("SICOOB") || upper.includes("EXTRATO DE CARTAO") || /\d{2}\/\d{2}\s*\|/.test(texto)) return "sicoob";
  if (upper.includes("NUBANK") || upper.includes("NU PAGAMENTOS") || /\d{2}\s+[A-Z]{3}\s+/.test(texto)) return "nubank";
  if (upper.includes("ITAU") || upper.includes("ITAÚ") || /ITAU\s+UNIBANCO/.test(upper)) return "itau";
  return "desconhecido";
}
...
export function parsearFaturaSicoob(texto: string, anoFatura: number): TransacaoParseada[] {
  ...
  for (const bloco of blocos) {
    ...
    const valorParcela = parseFloat(valorMatch[1].replace(/\./g, "").replace(",", "."));
    if (isNaN(valorParcela) || valorParcela <= 0 || valorParcela > 1000000) continue;
    let meio = trimmed.substring(trimmed.indexOf(" ")).trim();
    const idxValor = meio.lastIndexOf(valorMatch[1]);
    if (idxValor > -1) meio = meio.substring(0, idxValor).trim();
    ...
    const valor_total_compra = total_parcelas ? valorParcela * total_parcelas : null;
    const categoria_sugerida = sugerirCategoria(descricao);
    transacoes.push({
      id: crypto.randomUUID(), data, descricao,
      valor: valorParcela, parcela_atual, total_parcelas, valor_total_compra,
      categoria_sugerida, selecionada: true, isDuplicada: false,
    });
  }
  return transacoes;
}
...
export function parsearFaturaNubank(texto: string, anoFatura: number): TransacaoParseada[] {
  ...
  for (const rawLine of rawLines) {
    ...
    if (!line) continue;
    if (/^(NUBANK|NU PAGAMENTOS|FATURA|RESUMO|CARTAO|LIMITE|TOTAL|PARCELAS|COMPRAS|AJUSTES)/i.test(line)) continue;
    const isDateStart = /^\d{2}\s+[A-Z]{3}\b/i.test(line) || /^\d{2}\/\d{2}\b/.test(line);
    if (isDateStart) { if (currentBuffer) entryBuffers.push(currentBuffer); currentBuffer = line; }
    else if (currentBuffer) currentBuffer += " " + line;
  }
  ...
  for (const entry of entryBuffers) {
    ...
    if (match.length === 5 && /^[A-Z]{3}$/i.test(match[2])) {
      ...
      dia = parseInt(match[1]); mes = meses[match[2].toUpperCase()] || 1;
      descricao = match[3].trim(); valorStr = match[4];
    } else {
      dia = parseInt(match[1]); mes = parseInt(match[2]);
      descricao = match[3].trim(); valorStr = match[4];
    }
    ...
    const valorParcela = parseFloat(valorStr.replace(/\./g, "").replace(",", "."));
    if (isNaN(valorParcela) || valorParcela <= 0) continue;
    const valor_total_compra = total_parcelas ? valorParcela * total_parcelas : null;
    const categoria_sugerida = sugerirCategoria(descricao);
    transacoes.push({
      id: crypto.randomUUID(), data, descricao,
...
export function parsearFaturaItau(texto: string, anoFatura: number): TransacaoParseada[] {
  ...
  for (const rawLine of rawLines) {
    const line = rawLine.trim();
    if (!line) continue;
    if (/^(ITAU|ITAÚ|EXTRATO|CARTAO|FATURA|RESUMO|LIMITE|DISPONIVEL|VENCIMENTO|TOTAL|SAQUES|COMPRAS|PARCELAS)/i.test(line)) continue;
    if (/^\d{2}\/\d{2}/.test(line)) { if (currentBuffer) entryBuffers.push(currentBuffer); currentBuffer = line; }
    else if (currentBuffer) currentBuffer += " " + line;
  }
  if (currentBuffer) entryBuffers.push(currentBuffer);
  const transacoes: TransacaoParseada[] = [];
...
export function parsearFatura(texto: string, anoFatura: number): { banco: BancoDetectado; transacoes: TransacaoParseada[] } {
  ...
  switch (banco) {
    case "sicoob": return { banco, transacoes: parsearFaturaSicoob(texto, anoFatura) };
    case "nubank": return { banco, transacoes: parsearFaturaNubank(texto, anoFatura) };
    case "itau": return { banco, transacoes: parsearFaturaItau(texto, anoFatura) };
    default:
      const parsers = [parsearFaturaSicoob(texto, anoFatura), parsearFaturaNubank(texto, anoFatura), parsearFaturaItau(texto, anoFatura)];
...
export function useImportarFatura() {
  ...
  const cartoesQuery = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("contas_usuario").select("*").eq("tipo", "cartao_credito");
  ...
  const categoriasQuery = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("categorias").select("*").eq("tipo", "despesa");
      if (error) throw error;
      return data || [];
    },
    staleTime: 1000 * 60 * 5,
  ...
  const analisar = useCallback(async (texto: string, contaId: string, mesReferencia: string, vencimento: string, ignorarDuplicatas = false) => {
    ...
    try {
      ...
      if (parseadas.length === 0) {
        toast({ title: "Nenhuma transação encontrada", description: "Não foi possível extrair automaticamente. Tente:\n1. Copiar e colar o texto manualmente\n2. Verificar se o PDF não está protegido contra cópia\n3. Verificar se o banco está na lista suportada (Sicoob, Nubank, Itaú)", variant: "default" });
        setTransacoes([]);
        return;
      }
      const categoriasMap = new Map<string, string>();
      for (const cat of (categoriasQuery.data || [])) categoriasMap.set(cat.nome.toUpperCase(), cat.id);
      const parseadasComCat = parseadas.map(t => {
        if (t.categoria_sugerida) {
          ...
          if (catId) return { ...t, categoria_id: catId, categoria_nome: t.categoria_sugerida };
        }
        return t;
      });
      let comStatus = [];
      if (ignorarDuplicatas) {
        ...
        const { data: existentes } = await supabase
          .from("transacoes").select("descricao, valor, data, parcela_atual, total_parcelas, hash_importacao")
          .eq("tipo", "despesa").or(`cartao_id.eq.${contaId},conta_id.eq.${contaId}`)
          .gte("data", `${anoFatura}-01-01`).lte("data", `${anoFatura}-12-31`);
        comStatus = parseadasComCat.map(t => {
          const duplicada = existentes?.some(e => {
            const mesmaDesc = e.descricao?.toLowerCase().includes(t.descricao.toLowerCase()) || t.descricao.toLowerCase().includes(e.descricao?.toLowerCase() || "");
      ...
      toast({ title: `${parseadas.length} transações encontradas`, description: `Banco: ${banco.toUpperCase()}. ${comStatus.filter(t => t.isDuplicada).length} já existem.` });
    } catch (err) {
  ...
  const importar = useCallback(async (contaId: string, mesReferencia: string, vencimento: string, transacoesSelecionadas: TransacaoParseada[]) => {
    try {
      ...
      const hash = hashTransacoes(transacoesSelecionadas);
      const { data: importacao, error: importError } = await supabase
        .from("fatura_cartao_importacoes")
        .insert({ user_id: user.id, workspace_id: activeWorkspace?.id || null, conta_id: contaId, mes_referencia: mesReferencia, vencimento: vencimento || null, hash_transacoes: hash, transacoes_criadas: 0 })
        .select("id").single();
      if (importError) throw importError;
      const importacaoId = importacao.id;
      let criadas = 0;
      for (let i = 0; i < transacoesSelecionadas.length; i++) {
        const t = transacoesSelecionadas[i];
        if (!t.selecionada) continue;
        const { error } = await supabase.from("transacoes").insert({
          user_id: user.id, workspace_id: activeWorkspace?.id || null, tipo: "despesa",
          descricao: t.total_parcelas && t.total_parcelas > 1 ? `${t.descricao} (${t.parcela_atual}/${t.total_parcelas})` : t.descricao,
          valor: t.valor, data: t.data, mes_referencia: mesReferencia,
          categoria_id: t.categoria_id || null, cartao_id: contaId, conta_id: contaId,
          metodo_pagamento: "cartao_credito", parcela_atual: t.parcela_atual, total_parcelas: t.total_parcelas,
          importacao_id: importacaoId, hash_importacao: hash, numero_linha: i + 1,
        });
        if (error) throw error;
        criadas++;
      }
      await supabase.from("fatura_cartao_importacoes").update({ transacoes_criadas: criadas }).eq("id", importacaoId);
      await qc.invalidateQueries({ queryKey: ["transacoes"] });
      await qc.invalidateQueries({ queryKey: ["despesas"] });
      await qc.invalidateQueries({ queryKey: ["fatura_cartao_importacoes"] });
      toast({ title: "Importação concluída!", description: `${criadas} transações criadas.` });
      setTransacoes([]); setBancoDetectado("desconhecido");
      return criadas;
    } catch (err) {
  ...
  return {
    transacoes, valorTotalFatura: transacoes.reduce((acc, t) => acc + t.valor, 0),
    bancoDetectado, isAnalisando, isExtraindoPDF,
    cartoes: cartoesQuery.data || [], categorias: categoriasQuery.data || [],
    isLoadingCartoes: cartoesQuery.isLoading, isLoadingCategorias: categoriasQuery.isLoading,
    analisar, importar, extrairPDF, toggleSelecao, setCategoria, selecionarTodas, desselecionarDuplicadas,
