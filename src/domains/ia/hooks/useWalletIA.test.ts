...
vi.mock("../services/WalletAiOrchestratorClient", async () => {
  class WalletAiOrchestratorError extends Error {
    constructor(public readonly code: string, m: string) {
      super(m);
      this.name = "WalletAiOrchestratorError";
    }
  }
  class WalletAiOrchestratorClient {
...
const dados = {
  receitas: [{ valor: 15000, data: "2026-08-01", descricao: "V" }],
  despesas: [{ valor: 5000, data: "2026-08-01", descricao: "C" }],
  contas: [{ nome: "Conta", saldo_atual: 10000, tipo: "corrente" }],
};
const opts = { workspaceId: "ws-123", conversaId: "conv-123", dadosFinanceiros: dados, fastQueryFn: mockFast };
const ok = (c = "OK") => ({
  message: { role: "assistant" as const, content: c },
  toolCalls: [],
  iterations: 1,
  usage: {},
  estimatedCostUsd: 0,
...
describe("useWalletIA", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.sendMessage.mockResolvedValue(ok());
    mockFast.mockReturnValue("R$ 15.000 em receitas.");
  });
  ...
  describe("Workspace isolation", () => {
    ...
    it("sem workspaceId: erro sem inventar dados", async () => {
      ...
      expect(e?.content).toMatch(/workspace/i);
      expect(e?.content).not.toMatch(/R\$\s*\d/);
      expect(mocks.sendMessage).not.toHaveBeenCalled();
    });
  });
    ...
    it("loadHistory substitui mensagens", () => {
      ...
      act(() => {
        result.current.loadHistory([
          { id: "1", role: "user" as const, content: "Ola", createdAt: new Date() },
          { id: "2", role: "assistant" as const, content: "Oi", createdAt: new Date() },
        ]);
      });
