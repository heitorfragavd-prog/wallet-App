...
export const useIAConfiguracoes = () => {
  ...
  const fetchConfiguracao = async () => {
    try {
      ...
      logger.error('useIAConfiguracoes', 'Erro ao buscar configurações', { error: error instanceof Error ? error.message : String(error) });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const salvarConfiguracao = async (apiKey: string, modelo: string) => {
    try {
      ...
      logger.error('useIAConfiguracoes', 'Erro ao salvar configuração', { error: error instanceof Error ? error.message : String(error) });
      toast({
        title: "Erro",
