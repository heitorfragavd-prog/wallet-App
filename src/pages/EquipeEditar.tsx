...
import { ArrowLeft, Camera, Save, Trash2, UserCheck, CreditCard, Bus, PhoneCall, Home } from "lucide-react";
...
export default function EquipeEditarPage() {
  ...
  const { data: colaboradores, refetch } = useColaboradores();
  ...
  useEffect(() => {
    if (colaborador) {
      setForm({
        ...
        tipo: colaborador.tipo || "funcionario",
        cargo: colaborador.cargo || "",
        salario_bruto: String(colaborador.salario_bruto ?? ""),
        vale_transporte: String(colaborador.vale_transporte ?? "0"),
        vale_transporte_diario: String(colaborador.vale_transporte_diario ?? "0"),
        vale_refeicao: String(colaborador.vale_refeicao ?? "0"),
        outros_beneficios: String(colaborador.outros_beneficios ?? "0"),
        data_admissao: colaborador.data_admissao || "",
        carga_horaria_semanal: String(colaborador.carga_horaria_semanal ?? "44"),
        status: (colaborador.status as "ativo" | "experiencia") || "experiencia",
        cpf: colaborador.cpf || "",
        rg: colaborador.rg || "",
        ...
        contato_emergencia_1: colaborador.contato_emergencia_1 || "",
        contato_emergencia_2: colaborador.contato_emergencia_2 || "",
        pix_tipo: colaborador.pix_tipo || "cpf",
        ...
        linha_onibus: colaborador.linha_onibus || "",
        valor_passagem: String(colaborador.valor_passagem ?? "6.25"),
      });
      setFotoPreview(colaborador.foto_url);
      setFotoPosicao(colaborador.foto_posicao || "50% 15%");
    }
  }, [colaborador]);
  ...
  const handleSubmit = async () => {
    ...
    let fotoUrl = colaborador?.foto_url || null;
    if (fotoPreview && fotoPreview !== colaborador?.foto_url) {
      const fileName = `colaboradores/${activeWorkspace?.id || "default"}/${Date.now()}.jpg`;
      ...
      if (!upError) {
        ...
        toast({ title: "Aviso", description: "Foto não foi salva: " + upError.message, variant: "default" });
      }
    }
  ...
  const handleDelete = async () => {
    if (!confirm("Tem certeza que deseja excluir este colaborador? Todos os custos e presenças serão perdidos.")) return;
    setLoading(true);
  ...
  if (!colaborador) {
    return (
      <DashboardLayout>
        <div className="p-8 text-center text-muted-foreground">Carregando dados do colaborador...</div>
      </DashboardLayout>
    );
  }
      ...
      <div className="p-4 sm:p-6 max-w-3xl mx-auto space-y-6">
        ...
        <Card className="bg-card/60 border-border/40">
          <CardContent className="p-6 space-y-6">
            <div className="flex flex-col items-center gap-3">
              <Avatar className="h-28 w-28 border-2 border-primary/50 shadow-md shrink-0">
                ...
                <AvatarFallback className="bg-primary/20 text-primary text-2xl font-bold">
                  {form.nome ? form.nome.split(" ").map(n => n[0]).join("").substring(0, 2).toUpperCase() : <Camera className="h-8 w-8" />}
                </AvatarFallback>
              </Avatar>
              <Label htmlFor="foto" className="cursor-pointer text-sm text-primary hover:underline font-medium">
            ...
            <div className="space-y-4 pt-2 border-t border-border/30">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input type="number" value={form.salario_bruto} onChange={e => setForm({ ...form, salario_bruto: e.target.value })} />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input type="number" value={form.carga_horaria_semanal} onChange={e => setForm({ ...form, carga_horaria_semanal: e.target.value })} />
                </div>
                <div className="space-y-2">
            ...
            {/* Transporte & Benefícios (SEMPRE VISÍVEL) */}
            <div className="space-y-4 pt-4 border-t border-border/30">
              <h3 className="text-sm font-semibold text-primary flex items-center gap-2">
              ...
              {form.tipo !== "folguista" && (
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input type="number" step="0.01" value={form.vale_transporte_diario} onChange={e => setForm({ ...form, vale_transporte_diario: e.target.value })} />
                  </div>
                  <div className="space-y-2">
